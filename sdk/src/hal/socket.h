...
class _single_thread SocketAddress {
 ...
 public:
  SocketAddress();
  SocketAddress(const char* addrString, int port,
  ...
  virtual u_result setAddressFromString(const char* address_string,
                                        address_type_t = ADDRESS_TYPE_INET);
 ...
 public:
  static size_t LoopUpHostName(const char* hostname, const char* sevicename,
...
class SocketBase {
 public:
  ...
  virtual u_result setTimeout(_u32 timeout,
                              socket_direction_mask msk = SOCKET_DIR_BOTH) = 0;
...
class _single_thread StreamSocket : public SocketBase {
 public:
  ...
  static StreamSocket* CreateSocket(
  ...
  virtual u_result waitforIncomingConnection(
