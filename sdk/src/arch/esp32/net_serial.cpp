...
bool raw_serial::open()
{
    ...
    if (strstr(_portName, "0") || strstr(_portName, "UART_NUM_0")) _uart_num = 0;
    else if (strstr(_portName, "1") || strstr(_portName, "UART_NUM_1")) _uart_num = 1;
    else if (strstr(_portName, "2") || strstr(_portName, "UART_NUM_2")) _uart_num = 2;
    else _uart_num = atoi(_portName);
    ...
    if (uart_param_config((uart_port_t)_uart_num, &uart_config) != ESP_OK) return false;

    // Using default pins, users should use uart_set_pin() externally if needed
    if (uart_driver_install((uart_port_t)_uart_num, SERIAL_RX_BUFFER_SIZE * 2, 0, 0, NULL, 0) != ESP_OK) return false;
...
{
    // DTR is not a standard UART pin on ESP32, often used for motor control on RPLIDAR A1.
    // User needs to handle this via GPIO.
}
