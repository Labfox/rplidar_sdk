...
static int _inet_pton(int Family, const char* pszAddrString, void* pAddrBuf) {
  ...
  int result = WSAStringToAddressA((char*)pszAddrString, Family, NULL,
                                   (sockaddr*)&tmpholder, &actualSize);
  if (result) return -1;

  switch (Family) {
    case AF_INET: {
      struct sockaddr_in* ipv4 =
          reinterpret_cast<struct sockaddr_in*>(&tmpholder);
      memcpy(pAddrBuf, &(ipv4->sin_addr), sizeof(ipv4->sin_addr));
    } break;
    case AF_INET6: {
      struct sockaddr_in6* ipv6 =
          reinterpret_cast<struct sockaddr_in6*>(&tmpholder);
...
u_result SocketAddress::setAddressFromString(
  ...
  switch (type) {
    case ADDRESS_TYPE_INET:
      reinterpret_cast<sockaddr_storage*>(_platform_data)->ss_family = AF_INET;
      ans =
          _inet_pton(AF_INET, address_string,
 ...
 public:
  ...
  virtual u_result setTimeout(_u32 timeout, socket_direction_mask msk) {
    ...
    if (msk & SOCKET_DIR_RD) {
      ans = ::setsockopt(_socket_fd, SOL_SOCKET, SO_RCVTIMEO, (char*)&tv,
                         (int)sizeof(tv));
      if (ans) return RESULT_OPERATION_FAIL;
    }

    if (msk & SOCKET_DIR_WR) {
      ans = ::setsockopt(_socket_fd, SOL_SOCKET, SO_SNDTIMEO, (char*)&tv,
                         (int)sizeof(tv));
      if (ans) return RESULT_OPERATION_FAIL;
    }
  ...
  virtual u_result connect(const SocketAddress& pairAddress) {
    ...
    if (SOCKET_ERROR ==
        ioctlsocket(_socket_fd, (long)FIONBIO, &mode_notBlock)) {
      return RESULT_OPERATION_FAIL;
    }
  ...
  virtual u_result recv(void* buf, size_t len, size_t& recv_len) {
    ...
    //::setsockopt(_socket_fd, IPPROTO_TCP, TCP_QUICKACK,  (const char *)1,
    //:sizeof(int)); :setsockopt(_socket_fd, IPPROTO_TCP, TCP_QUICKACK,
    //:(int[]){1}, sizeof(int))
    if (ans == SOCKET_ERROR) {
      recv_len = 0;
      switch (WSAGetLastError()) {
  ...
  virtual u_result enableKeepAlive(bool enable) {
    ...
    return ::setsockopt(_socket_fd, SOL_SOCKET, SO_KEEPALIVE,
                        (const char*)&bool_true, (int)sizeof(bool_true))
  ...
  virtual u_result enableNoDelay(bool enable) {
    int bool_true = enable ? 1 : 0;
    return ::setsockopt(_socket_fd, IPPROTO_TCP, TCP_NODELAY,
                        (const char*)&bool_true, (int)sizeof(bool_true))
               ? RESULT_OPERATION_FAIL
               : RESULT_OK;
  }
 ...
 public:
  DGramSocketImpl(SOCKET fd) : _socket_fd(fd) {
    ...
    ::setsockopt(_socket_fd, SOL_SOCKET, SO_REUSEADDR | SO_BROADCAST,
                 (char*)&bool_true, (int)sizeof(bool_true));
  ...
  virtual u_result setTimeout(_u32 timeout, socket_direction_mask msk) {
    ...
    if (msk & SOCKET_DIR_RD) {
      ans = ::setsockopt(_socket_fd, SOL_SOCKET, SO_RCVTIMEO, (const char*)&tv,
                         (int)sizeof(tv));
      if (ans) return RESULT_OPERATION_FAIL;
    }

    if (msk & SOCKET_DIR_WR) {
      ans = ::setsockopt(_socket_fd, SOL_SOCKET, SO_SNDTIMEO, (const char*)&tv,
                         (int)sizeof(tv));
      if (ans) return RESULT_OPERATION_FAIL;
    }
  ...
  virtual u_result setPairAddress(const SocketAddress* pairAddress) {
    ...
    const struct sockaddr* addr =
        pairAddress ? reinterpret_cast<const struct sockaddr*>(
  ...
  virtual u_result sendTo(const SocketAddress* target, const void* buffer,
                          size_t len) {
    const struct sockaddr* addr =
        target ? reinterpret_cast<const struct sockaddr*>(
                     target->getPlatformData())
               : NULL;
  ...
  virtual u_result recvFrom(void* buf, size_t len, size_t& recv_len,
    ...
    int ans = ::recvfrom(_socket_fd, (char*)buf, (int)len, 0, addr,
                         addr ? &source_addr_size : NULL);
    if (ans == SOCKET_ERROR) {
      recv_len = 0;
